        ...
        <>
          <div className="flex items-center justify-between gap-2 mb-2">
            {isUnassigned ? (
              ...
              <div className="relative inline-flex items-center" onClick={e => e.stopPropagation()}>
                <select id={`select-card-status-${task.id}`} value={task.status} onChange={e => handleStatusChange(e.target.value as TaskStatus)} className={`appearance-none text-[11px] font-bold px-2.5 py-0.5 pr-5 rounded-full border cursor-pointer transition-colors focus:outline-none ${isPending ? 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200/80' : isInProgress ? 'bg-sky-50 text-sky-700 border-sky-200 hover:bg-sky-100' : isResolved ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100'}`} title="Cambiar estado de la tarea">
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
            ...
            <div className="flex items-center gap-1.5 shrink-0" onClick={e => e.stopPropagation()}>
              {isUnassigned && permissions.canClaimTask && onClaim && <button type="button" id={`btn-me-hago-cargo-${task.id}`} onClick={e => { e.stopPropagation(); handleClaim(); }} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-[#142136] hover:bg-emerald-600 text-white transition-all shadow-2xs active:scale-[0.98] cursor-pointer" title="Hacerme cargo de esta tarea"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /><span>Me hago cargo</span></button>}
              {!isUnassigned && isResolved && permissions.canResolveTask && onStatusChange && <button type="button" id={`btn-reabrir-${task.id}`} onClick={e => { e.stopPropagation(); handleStatusChange('EN_PROCESO'); }} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-medium text-slate-500 hover:text-slate-800 hover:bg-slate-100 border border-slate-200/60 transition-colors cursor-pointer" title="Reabrir tarea"><RotateCcw className="w-3 h-3 text-slate-400" /><span>Reabrir</span></button>}
              {!isUnassigned && !isResolved && permissions.canReleaseTask && onRelease && <button type="button" id={`btn-liberar-${task.id}`} onClick={e => { e.stopPropagation(); handleRelease(); }} className="text-[11px] text-slate-500 hover:text-rose-600 hover:underline px-1 py-0.5 transition-colors cursor-pointer" title="Liberar tarea (vuelve a quedar disponible para el equipo)">Liberar</button>}
              {!isUnassigned && !isResolved && permissions.canResolveTask && onStatusChange && <button type="button" id={`btn-resolver-${task.id}`} onClick={e => { e.stopPropagation(); handleStatusChange('RESUELTA'); }} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 transition-colors cursor-pointer" title="Marcar como resuelta"><CheckCircle2 className="w-3.5 h-3.5" /><span>Resolver</span></button>}
            </div>
          </div>
        </>
      )}
